...
class FlagManager(AxisManager):
    ...
    def wrap(self, name, data, axis_map=None, **kwargs):
        ...
        if axis_map is None:
            if self[self._dets_name].count == self[self._samps_name].count:
                raise ValueError(
                    "Cannot auto-detect axis_map when dets and "
            ...
            if len(s) == 1:
                if s[0] == self[self._dets_name].count:
                    ...
                    raise ValueError(
                        "FlagManager only takes data aligned with"
                        " dets and/or samps. Data of shape {}"
                        " is the wrong shape".format(s)
                    )
            elif len(s) == 2:
                if (
                    s[0] == self[self._dets_name].count
                    and s[1] == self[self._samps_name].count
                ):
                    axis_map = [(0, self._dets_name), (1, self._samps_name)]
                elif (
                    s[1] == self[self._dets_name].count
                    and s[0] == self[self._samps_name].count
                ):
                    raise ValueError(
        ...
        if len(axis_map) == 1 and axis_map[0][1] == self._dets_name:
            ...
            data = RangesMatrix(
                [Ranges.ones_like(x) if Y else Ranges.zeros_like(x) for Y in data]
            )
            axis_map = [(0, self._dets_name), (1, self._samps_name)]
    ...
    def wrap_dets(self, name, data):
        ...
        if not len(s) == 1 or s[0] != self[self._dets_name].count:
            raise ValueError(
                "Data of shape {} is cannot be aligned with"
    ...
    def wrap_samps(self, name, data):
        ...
        if not len(s) == 1 or s[0] != self[self._samps_name].count:
            raise ValueError(
                "Data of shape {} is cannot be aligned with" "the samps axis".format(s)
            )
    ...
    def wrap_dets_samps(self, name, data):
        ...
        if (
            not len(s) == 2
            or s[0] != self[self._dets_name].count
            or s[1] != self[self._samps_name].count
        ):
            raise ValueError(
                "Data of shape {} is cannot be aligned with"
    ...
    def get_zeros(self, wrap=None):
        ...
        """
        out = RangesMatrix(
            [Ranges(self[self._samps_name].count) for det in self[self._dets_name].vals]
        )
        if not wrap is None:
    ...
    def promote(cls, src, dets_name, samps_name):
        ...
        """
        flagman = cls(src._axes.pop(dets_name), src._axes.pop(samps_name))
...
def has_all_cut(flag):
    return np.array([len(x.complement().ranges()) == 0 for x in flag], dtype="bool")
