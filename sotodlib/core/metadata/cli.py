...
from .. import context
from . import ObsDb, manifest, obsdb, obsfiledb
...
def get_parser():
    ...
    sp = sps.add_parser(
        "context",
        description="Inspect a context file and print out a few things about it.",
    )
    ...
    sp = sps.add_parser(
        "obsdb",
        description="Inspect an ObsDb.  By default, prints out summary information.  "
    ...
    sp.add_argument(
        "--type",
        choices=["context", "db"],
        help="Specifies how to interpret the file (to override guessing "
        "based on the filename extension.",
    )
    sp.add_argument("--list", "-l", action="store_true", help="List all observations")
    sp.add_argument(
        "--query",
        "-q",
        default="1",
        help="Restrict listed items with an ObsDb query string.",
    )
    sp.add_argument(
        "--key",
        "-k",
        action="append",
        default=[],
        help="If listing observations, also include the speficied db fields "
    ...
    sp.add_argument(
        ...
        action="append",
        default=[],
        help="Add a tag as an eligible column for query.",
    )
    ...
    sp = sps.add_parser(
        "obsfiledb",
        description="Inspect an ObsFileDb.  This can be used to list all files, "
    ...
    if args._subcmd == "context":
        ...
        if args.obs_id is None:
            if args.list:
                ...
                if args.query != "1":
                    print(
                        'Note the "--query" argument is not applied to these results!'
                    )
                    print()
