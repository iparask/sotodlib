...
class DataG3Module(object):
    ...
    def __init__(self, input: str = "signal", output: Optional[str] = "signal_out"):
        self.input = input
        if output is None:
    ...
    def __call__(self, f: spt3g.core.G3FrameType.Scan):
        if f.type == core.G3FrameType.Scan:
            if self.input not in f.keys() or f[self.input] is not core.G3TimestreamMap:
                raise ValueError(
                    """Frame is a Scan but does not have a G3Timestream map 
                                 named {}""".format(
                        self.input
                    )
                )
    ...
    def apply(self, f):
        ...
        if type(f) == core.G3Frame:
            if f.type != core.G3FrameType.Scan:
                raise ValueError(
                    """Cannot apply {} on a {} frame""".format(self.__class__, f.type)
                )
            self.__call__(f)
            return f
        ...
        raise ValueError(
            "apply requires a G3Frame or a G3TimestreamMap, you gave me {}".format(
                type(f)
            )
        )
