...
def expand_RangesMatrix(flat_rm):
    ...
    shape, intervals, ends = [flat_rm[k] for k in ["shape", "intervals", "ends"]]
    if len(shape) == 1:
    ...
    for i in range(0, len(ends), stride):
        ...
        ranges.append(
            expand_RangesMatrix({"shape": shape[1:], "intervals": _i, "ends": _e})
        )
...
def expand_csr_array(flat_arr):
    ...
    return csr_array(
        (flat_arr["data"], flat_arr["indices"], flat_arr["indptr"]),
...
def _save_axisman(axisman, dest, group=None, overwrite=False, compression=None):
    ...
    for item in schema:
        ...
        if item["encoding"] == "scalar":
            ...
            dest.create_dataset(
                item["name"], data=_retype_for_write(data), compression=compression
            )
...
def _load_axisman(src, group=None, cls=None, fields=None):
    ...
    for item in schema:
        ...
        if item["encoding"] == "axis":
            ...
            axisman.wrap(
                item["name"],
                _retype_for_read(src[item["name"]][:]) << u.Unit(units[item["name"]]),
                assign,
            )
        elif item["encoding"] == "scalar_quantity":
            axisman.wrap(
                item["name"],
                scalars[item["name"]] << u.Unit(units[item["name"]]),
                assign,
            )
        elif item["encoding"] == "axisman":
